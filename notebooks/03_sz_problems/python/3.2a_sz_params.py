...
#
# The only difference in a subduction zone problem is that each of these steps is more complicated than in the earlier examples.  Here we split steps 1-7 up across several notebooks.  In the [first](./3.2b_sz_slab.ipynb) we implement a python function to describe the slab surface using a spline.  The remaining details of the geometry are constructed in a function defined in the [next notebook](./3.2c_sz_geometry.ipynb).  Finally we implement a series of python classes to describe the remaining steps, 2-7, of the problem across subsequent notebooks.

# %% [markdown]
# ### Geometry
#
# Throughout our implementation, in the following notebooks, we will demonstrate its functionality using the simplified geometry previously laid out and repeated below in Figure 3.2.1. However our implementation will be applicable to the broader range of geometries and setups seen in the [global suite](../04_global_suites/4.1_global_suites_intro.ipynb).
#
# ![Figure 8a of Wilson & van Keken, 2023](images/benchmarkgeometry.png)
# *Figure 3.2.1: Geometry and coefficients for a simplified 2D subduction zone model. All coefficients and parameters are nondimensional. The decoupling point is indicated by the circle on the slab.*
...
params_filename = os.path.join(basedir, os.path.pardir, "data", "default_params.json")
